...
#nohup $cleos push action eosio.token transfer '[ "#", "#to", "1.0000 EOS", "" ]' -p "#"@active  >$LOG_FILE 2>&1 &
#donei

for i in `cat accounts.txt`;
do
nohup $cleos push action eosio.token transfer "[ \"$i\", \"${i}to\", \"1.0000 EOS\", \"\" ]" -p ${i}@active  >$LOG_FILE 2>&1 &
done
